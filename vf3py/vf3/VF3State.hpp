...
namespace vflib
{
	...
	private:
		...
		nodeID_t start_target_node_id;  //Starting node ID in the target graph
	...
	public:
		...
		VF3State(ARGraph<Node1, Edge1> *g1, ARGraph<Node2, Edge2> *g2,
			...
			nodeID_t* order = NULL, bool induced = true, nodeID_t start_target_node_id = NULL_NODE);
	...
	template <typename Node1, typename Node2,
		...
		VF3State<Node1, Node2, Edge1, Edge2, NodeComparisonFunctor, EdgeComparisonFunctor>
			::VF3State(ARGraph<Node1, Edge1> *ag1, ARGraph<Node2, Edge2> *ag2, 
				uint32_t* class_1, uint32_t* class_2, uint32_t nclass, nodeID_t* order, bool induced, nodeID_t start_target_node_id)
			:State(ag1->NodeCount(), ag2->NodeCount(), order, induced)
	{
		...
		this->start_target_node_id = start_target_node_id;
		...
		::VF3State(const VF3State &state):State(state)
	{
		...
		start_target_node_id = state.start_target_node_id;
			...
			EdgeComparisonFunctor>::NextPair(nodeID_t *pn1, nodeID_t *pn2, nodeID_t prev_n1, nodeID_t prev_n2)
	{
		...
		else
		{
			...
			if (prev_n2 == NULL_NODE)
			{
				// Use start_target_node_id if specified, otherwise start from 0
				if (start_target_node_id != NULL_NODE && start_target_node_id < n2)
					prev_n2 = start_target_node_id;
				else
					prev_n2 = 0;
			}
			else
