	...
	bool ProcessState(VFState *s, ThreadId thread_id)
	{
		...
		if (s->IsGoal())
		{
			...
			if(storeSolutions)
			{
				...
				s->GetCoreSet(sol);
				solutions_lock.lock();
				solutions.push_back(sol); // TODO Problem 1.
...
public:
	...
	{
		if(lockFree) {
			delete (LockFreeStack<VFState*>*)globalStateStack;
		} else {
			delete (SynchronizedStack<VFState*>*)globalStateStack;
		}
		globalStateStack = NULL;
	}
