...
struct OptionStructure
{
	...
	OptionStructure():
		...
		start_target_node_id(UINT32_MAX){}
};
...
template <typename Node, typename Edge>
vflib::MatchingEngine<state_t<Node, Edge>>* CreateMatchingEngine(const Options& opt)
{
#ifdef VF3P
	switch(opt.algo)
	{
		case VF3PGSS:
			return new vflib::ParallelMatchingEngine<state_t<Node, Edge> >(opt.numOfThreads, opt.storeSolutions, opt.lockFree, opt.cpu);
		case VF3PWLS:
			return new vflib::ParallelMatchingEngineWLS<state_t<Node, Edge> >(opt.numOfThreads, opt.storeSolutions, opt.lockFree,
    ...
    return new vflib::MatchingEngine<state_t<Node, Edge> >(opt.storeSolutions);
#endif
}
