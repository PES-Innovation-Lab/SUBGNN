    ...
    RestartsSchedule & restarts_schedule) -> SearchResult
{
    ...
    // If we have a starting target vertex, prioritize it if it's in the current domain
    if (model.has_start_from_target_vertex()) {
        int start_target_vertex = model.get_start_from_target_vertex();
        
        // Find the starting vertex in branch_v and move it to the front
        for (unsigned i = 0; i < branch_v_end; ++i) {
            if (branch_v[i] == start_target_vertex) {
                // Move the starting vertex to the front
                int temp = branch_v[i];
                for (unsigned j = i; j > 0; --j) {
                    branch_v[j] = branch_v[j-1];
                }
                branch_v[0] = temp;
                break;
            }
...
auto HomomorphismSearcher::find_branch_domain(const Domains & domains) -> const HomomorphismDomain *
{
    ...
    // If we have a starting target vertex, prioritize a pattern vertex that can map to it
    if (model.has_start_from_target_vertex()) {
        int start_target_vertex = model.get_start_from_target_vertex();
        
        // First pass: look for domains that can map to the starting target vertex
        for (auto & d : domains) {
            if (! d.fixed && d.values.test(start_target_vertex)) {
                if ((! result) ||
        ...
        // If we found a suitable domain that can map to the starting vertex, return it
        if (result != nullptr) {
            return result;
        }
    }
    
    // Default behavior: find the smallest domain or highest degree if tied
    for (auto & d : domains)
        if (! d.fixed)
            if ((! result) ||
                (d.count < result->count) ||
