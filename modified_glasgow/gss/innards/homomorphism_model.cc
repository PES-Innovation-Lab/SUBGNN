...
struct HomomorphismModel::Imp
{
    ...
    // Starting vertex information
    optional<int> start_from_target_vertex_id;
    ...
    target_size(target.size())
{
    ...
    // Check if start_from_target_vertex is specified and valid
    if (_imp->params.start_from_target_vertex) {
        auto start_vertex_id = target.vertex_from_name(*_imp->params.start_from_target_vertex);
        if (start_vertex_id) {
            _imp->start_from_target_vertex_id = *start_vertex_id;
        }
        else {
            throw UnsupportedConfiguration{"Start target vertex '" + *_imp->params.start_from_target_vertex + "' not found in target graph"};
        }
    }
}
...
auto HomomorphismModel::has_start_from_target_vertex() const -> bool
{
    return _imp->start_from_target_vertex_id.has_value();
}

auto HomomorphismModel::get_start_from_target_vertex() const -> int
{
    if (!_imp->start_from_target_vertex_id.has_value()) {
        throw std::runtime_error("No start target vertex specified");
    }
    return *_imp->start_from_target_vertex_id;
}
