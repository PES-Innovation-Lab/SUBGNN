...
auto main(int argc, char * argv[]) -> int
{
    try {
        ...
        po::options_description starting_options{"Starting vertex options"};
        starting_options.add_options()                                                                                     //
            ("start-from-target", po::value<string>(), "Start search from this target vertex (improves search in nearby locality)");
        display_options.add(starting_options);
        ...
        // Set starting target vertex if specified
        if (options_vars.count("start-from-target")) {
            params.start_from_target_vertex = options_vars["start-from-target"].as<string>();
            cout << "start_from_target_vertex = " << *params.start_from_target_vertex << endl;
        }
